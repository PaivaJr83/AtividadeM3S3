...
class CalculadorFrete:
    def ler_dimensoes_objeto(self, altura, comprimento, largura):
        volume = altura * comprimento * largura
    ...
    def calcular_preco_volume(self, volume):
        if volume < 1000:
            return 10.0
        elif volume < 10000:
            return 20.0
    ...
    def validar_medida(self, medida):
        try:
            ...
            raise ValueError('Você digitou alguma dimensão do objeto com valor não numérico')

    def ler_peso_objeto(self, peso):
        if peso > 30:
            raise ValueError('Não é aceito objetos tão pesados')
        return peso

    def calcular_multiplicador_peso(self, peso):
        if peso < 0.1:
            return 1.0
        elif peso < 1:
            return 1.5
    ...
    def ler_rota(self, rota):
        rotas_validas = ['br', 'bs', 'rb', 'rs', 'sr', 'sb']
        if rota.lower() not in rotas_validas:
            raise ValueError('Você digitou uma rota que não existe')
        return rota.lower()

    def calcular_multiplicador_rota(self, rota):
        if rota in ['rs', 'sr']:
            return 1.0
        elif rota in ['bs', 'sb']:
            return 1.2
    ...
    def calcular_frete(self, volume, multiplicador_peso, multiplicador_rota):
        return volume * multiplicador_peso * multiplicador_rota
